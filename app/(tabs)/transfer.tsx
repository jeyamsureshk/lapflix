...
import { ArrowLeft, Download, Upload, CheckCircle2, AlertCircle, FileText, Trash2, ExternalLink, Pause, Play } from 'lucide-react-native';
      ...
      <View style={styles.cardHeader}>
        ...
        <View style={styles.fileMeta}>
          ...
          <Text style={styles.fileType}>
            {item.type === 'download' ? 'Download' : 'Upload'} 
        ...
        {item.status === 'paused' && <Pause size={20} color={Colors.warning[600]} />}
      </View>
          ...
          <View style={styles.progressBarTrack}>
            <View 
              style={[
                ...
                { width: `${Math.round(item.progress * 100)}%`, backgroundColor: item.status === 'paused' ? Colors.warning[500] : Colors.primary[600] }
              ]} 
            />
          </View>
      ...
      <View style={styles.cardFooter}>
        {item.status === 'completed' && item.uri ? (
          <View style={styles.completedWrap}>
            ...
            {item.type === 'download' && (
              <Text style={styles.savedPath} numberOfLines={2}>
                Saved to: {decodeURIComponent(item.uri.replace('file://', ''))}
              </Text>
            )}
          </View>
        ) : (item.status === 'active' || item.status === 'paused') ? (
          <View style={styles.controlsWrap}>
             {item.status === 'active' ? (
                <Pressable style={styles.actionBtn} onPress={() => transferManager.pauseTransfer(item.id)}>
                  <Pause size={14} color={Colors.warning[600]} />
                  <Text style={[styles.actionBtnText, { color: Colors.warning[600] }]}>Pause</Text>
                </Pressable>
             ) : (
                <Pressable style={styles.actionBtn} onPress={() => transferManager.resumeTransfer(item.id)}>
                  <Play size={14} color={Colors.primary[600]} />
          ...
          <View style={styles.completedWrap}>
             <Text style={styles.errorPath} numberOfLines={2}>
                {item.error || 'Transfer failed due to a network error.'}
             </Text>
          </View>
        ) : null}
      </View>

      <Pressable style={styles.dismissBtn} onPress={() => transferManager.cancelTransfer(item.id)}>
    ...
    <View style={styles.screen}>
      ...
      {transfers.length === 0 ? (
        <View style={styles.emptyContainer}>
          ...
          <Text style={styles.emptySubtitle}>Files you download or upload will appear here.</Text>
        </View>
      ) : (
        <FlatList
          data={transfers}
...
const styles = StyleSheet.create({
  ...
  cardFooter: { flexDirection: 'row', borderTopWidth: 1, borderTopColor: Colors.neutral[100], paddingTop: 10, marginTop: 4, justifyContent: 'flex-start' },
  controlsWrap: { flexDirection: 'row', gap: 16 },
  completedWrap: { flex: 1, gap: 6, alignItems: 'flex-start' },
  savedPath: { fontSize: 11, color: Colors.neutral[400], lineHeight: 16 },
  errorPath: { fontSize: 12, color: Colors.error[600], lineHeight: 16, fontWeight: '500' },
